...
public final class TinkerIndexHelper {
    ...
    public static List<RocksVertex> queryVertexIndex(final AbstractTinkerGraph graph, final String key, final Object value) {
        return null == graph.vertexIndex ? Collections.emptyList() : graph.vertexIndex.get(key, value);
    }

    public static List<RocksEdge> queryEdgeIndex(final AbstractTinkerGraph graph, final String key, final Object value) {
        return null == graph.edgeIndex ? Collections.emptyList() : graph.edgeIndex.get(key, value);
    }

    public static void autoUpdateIndex(final RocksEdge edge, final String key, final Object newValue, final Object oldValue) {
    ...
    public static void autoUpdateIndex(final RocksVertex vertex, final String key, final Object newValue, final Object oldValue) {
    ...
    public static void removeElementIndex(final RocksVertex vertex) {
        final AbstractTinkerGraph graph = (AbstractTinkerGraph) vertex.graph();
        if (graph.vertexIndex != null)
            graph.vertexIndex.removeElement(vertex);
    }

    public static void removeElementIndex(final RocksEdge edge) {
        final AbstractTinkerGraph graph = (AbstractTinkerGraph) edge.graph();
        if (graph.edgeIndex != null)
            graph.edgeIndex.removeElement(edge);
    }

    public static void removeIndex(final RocksVertex vertex, final String key, final Object value) {
        final AbstractTinkerGraph graph = (AbstractTinkerGraph) vertex.graph();
        if (graph.vertexIndex != null)
            graph.vertexIndex.remove(key, value, vertex);
    }

    public static void removeIndex(final RocksEdge edge, final String key, final Object value) {
        final AbstractTinkerGraph graph = (AbstractTinkerGraph) edge.graph();
        if (graph.edgeIndex != null)
